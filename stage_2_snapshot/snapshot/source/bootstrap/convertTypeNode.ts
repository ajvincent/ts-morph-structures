...
export default function convertTypeNode(
  ...
  if (Node.isTypeQuery(typeNode)) {
    ...
    const structure =
      typeof structureMaybeString === "string"
  ...
  if (Node.isTypePredicate(typeNode)) {
    const parameterNode: Identifier | ThisTypeNode =
      typeNode.getParameterNameNode();
    ...
    if (isType_node) {
      isType_TypeStructure = convertTypeNode(
        isType_node,
        consoleTrap,
        subStructureResolver,
      );
  ...
  // identifiers, type-argumented type nodes
    ...
    const objectType =
      typeof objectTypeMaybeString === "string"
