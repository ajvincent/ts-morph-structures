...
> {
  static memberKey(
    member: Exclude<ClassMemberImpl, ConstructorDeclarationImpl>,
  ): string {
    let rv = "";
    if (member.isStatic) rv = "static ";
    if (member.kind === StructureKind.GetAccessor) rv += "get ";
    else if (member.kind === StructureKind.SetAccessor) rv += "set ";
    rv += member.name;
    return rv;
  }
