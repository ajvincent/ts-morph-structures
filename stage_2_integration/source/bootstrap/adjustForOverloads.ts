 ...
 * @internal
 */
export function fixFunctionOverloads(
...
): void
{
  if (Structure.isStatemented(structure) && Array.isArray(structure.statements)) {
    ...
    structure.statements = structure.statements.reduceRight(
      (collectedStatements: IteratedStatement[], statement: IteratedStatement): IteratedStatement[] => {
        return prependOverloadsOfKindInside<FunctionDeclarationStructure, IteratedStatement>(
    ...
    if (structure.methods && structure.methods.length > 0) {
      ...
      structure.methods = (structure.methods as MethodDeclarationStructure[]).reduceRight(
        (
          collectedMethods: MethodDeclarationStructure[],
          method: MethodDeclarationStructure
        ): MethodDeclarationStructure[] => {
          return prependOverloadsOfKindInside<MethodDeclarationStructure, MethodDeclarationStructure>(
    ...
    if (structure.ctors && structure.ctors.length > 0) {
      ...
      structure.ctors = (structure.ctors as ConstructorDeclarationStructure[]).reduceRight(
        (
          collectedConstructors: ConstructorDeclarationStructure[],
          ctor: ConstructorDeclarationStructure
        ): ConstructorDeclarationStructure[] => {
          return prependOverloadsOfKindInside<ConstructorDeclarationStructure, ConstructorDeclarationStructure>(
...
type IteratedStatement = IterableElement<StatementedNodeStructure["statements"]>;
...
>
{
  ...
  constructor(
    structure: DeclarationStructure
  )
  {
    this.isStatic = structure.kind === StructureKind.Method && (structure.isStatic ?? false);
    this.kind = structure.kind;
    this.name = structure.kind === StructureKind.Constructor ? "constructor" : structure.name;

    this.structure = structure;
  }

  isEquivalent(other: CallableDescription<DeclarationStructure>): boolean {
    return this.isStatic === other.isStatic && this.kind === other.kind && this.name === other.name;
  }
}
...
{
  return (typeof statement !== "object") || (statement.kind !== StructureKind.FunctionOverload);
}

function prependOverloadsOfKindInside<
  DeclarationStructure extends FunctionDeclarationStructure | MethodDeclarationStructure | ConstructorDeclarationStructure,
  StatementType extends IteratedStatement | MethodDeclarationStructure | ConstructorDeclarationStructure
>
(
...
): StatementType[]
{
  ...
  if (callable.kind !== StructureKind.Constructor && callable.name === undefined) {
    collectedFromBack.unshift(statement);
    lastCallableWrapper.lastCallable = undefined;
    return collectedFromBack;
  }
  ...
  if (parentStructure.kind === StructureKind.Function) {
    ...
    prependOverload<FunctionDeclarationStructure, FunctionDeclarationOverloadStructure>(
    ...
    prependOverload<MethodDeclarationStructure, MethodDeclarationOverloadStructure>(
    ...
    prependOverload<ConstructorDeclarationStructure, ConstructorDeclarationOverloadStructure>(
...
function prependOverload<
  ParentStructure extends FunctionDeclarationStructure | MethodDeclarationStructure | ConstructorDeclarationStructure,
  OverloadStructure extends FunctionDeclarationOverloadStructure | MethodDeclarationOverloadStructure | ConstructorDeclarationOverloadStructure
>
(
