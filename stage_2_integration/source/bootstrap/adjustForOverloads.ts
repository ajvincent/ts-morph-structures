...
): void
{
  if (Structure.isStatemented(structure) && Array.isArray(structure.statements)) {
    ...
    for (let i = structure.statements.length - 1; i >= 0; i--) {
      const statement: string | Structures | WriterFunction = structure.statements[i];
      if (typeof statement !== "object") {
      ...
      if (lastNamedFunction === undefined || lastNamedFunction.name !== statement.name) {
        lastNamedFunction = statement;
        continue;
      }

      assert(
        lastNamedFunction.overloads === undefined || lastNamedFunction.overloads.length === 0,
        "why does a function with the same name in this statement block have overloads?"
      );
      assert(
        lastNamedFunction.statements === undefined || lastNamedFunction.statements.length === 0,
      ...
      const overload = statement as unknown as FunctionDeclarationOverloadStructure;
