...
>
{
  ...
  >(
    kind: Kind,
    structure: OptionalKind<StructureType>,
  ): ResultType
  {
  ...
  cloneRequiredAndOptionalArray<
    ...
    RequiredTargetType extends Extract<StructureImpls, KindedStructure<RequiredSourceKind>>,
    OptionalTargetType extends Extract<StructureImpls, KindedStructure<OptionalSourceKind>>,
  >
  (
  ...
  {
    const sourceArray = this.forceArray<RequiredSourceType | OptionalSourceType>(
      sources
    );

    return sourceArray.map(
      sourceValue => this.#cloneRequiredOrOptionalStructure<
        RequiredSourceType,
        RequiredSourceKind,
        OptionalSourceType,
        OptionalSourceKind,
  ...
  ): RequiredTargetType | OptionalTargetType
  {
    ...
    return this.#cloneWithKind<
      OptionalSourceType, OptionalSourceKind, OptionalTargetType
