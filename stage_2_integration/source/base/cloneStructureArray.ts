...
): readonly (TargetType | stringOrWriterFunction)[]
{
  ...
  return sourceArray.map(
    sourceValue => cloneStructureStringOrWriter<
      SourceType, Kind, TargetType
...
): readonly (TargetType | string)[]
{
  ...
  return sourceArray.map(
    sourceValue => cloneStructureOrString<
      SourceType, Kind, TargetType
    >(sourceValue, sourceKind)
  );
}
...
): readonly TargetType[]
{
  ...
  return sourceArray.map(sourceValue => cloneStructure<SourceType, Kind, TargetType>(sourceValue, sourceKind));
}
